dep 'dotfiles', :git_repo do
  requires 'grc.managed'
  requires 'coreutils.managed'
  requires 'dotfiles installed'.with(:git_repo => git_repo)
  requires 'zsh'
end

meta :dotfile do
  accepts_value_for :configs
  accepts_value_for :chmod

  def source
    Dir.glob("#{ENV['HOME']}/.dotfiles/**/#{basename}.erb").first
  end

  def target
    "#{ENV['HOME']}/.#{basename}"
  end

  template {
    met? {
      source && Babushka::Renderable.new(target).from?(source.p)
    }
    meet {
      if source && source.p.exists?
        if chmod
          render_erb source, :to => target, :perms => chmod
        else
          render_erb source, :to => target
        end
      else
        log_error "Could not find #{basename}.erb template in ~/.dotfiles"
      end
    }
  }
end

dep 'dotfiles installed', :git_repo do
  requires 'dotfiles.cloned'.with(:git_repo => git_repo)
  requires 'dotfiles symlinked'
  requires 'gitconfig.dotfile'
end

dep 'dotfiles.cloned', :git_repo do
  git_repo.ask("Enter dotfiles repo location").default("git://github.com/sporkd/dotfiles.git")
  repo git_repo
  destination '~/.dotfiles'
end

dep 'gitconfig.dotfile', :full_name, :email do
  full_name.ask("Your full name (for gitconfig)")
  email.ask("Your email (for gitconfig)")
  configs({
    :full_name => full_name,
    :email => email
  })
end

dep 'dotfiles symlinked' do
  def home
    ENV['HOME']
  end

  def symlink_pattern
    "#{home}/.dotfiles/*/**{.symlink}"
  end

  met? {
    symlinks = Dir.glob(symlink_pattern).collect do |source|
      filename = source.split('/').last.split('.symlink').last
      target = "#{home}/.#{filename}"
      target.p.exists? && target.p.symlink?
    end
    symlinks.empty? || (symlinks.uniq == [true])
  }

  meet {
    Dir.glob(symlink_pattern).each do |source|
      filename = source.split('/').last.split('.symlink').last
      target = "#{home}/.#{filename}"
      if target.p.exists? && !target.p.symlink?
        shell "mv '#{home}/.#{filename}' '#{home}/.#{filename}.backup'"
      end
      shell "ln -sf '#{source}' '#{target}'"
    end
  }
end

dep 'dotfiles uninstalled' do
  def home
    ENV['HOME']
  end

  def dotfile_pattern
    "#{home}/.dotfiles/*/**{.symlink,.erb}"
  end

  def installed?(path)
    path.symlink? || (path.file? && !path.grep(/Generated by babushka/).nil?)
  end

  met? {
    dotfiles = Dir.glob(dotfile_pattern).collect do |source|
      filename = source.split('/').last.split('.')
      filename.pop
      target = "#{home}/.#{filename}"
      installed?(target.p)
    end
    dotfiles.empty? || (dotfiles.uniq == [false])
  }

  meet {
    Dir.glob(dotfile_pattern).each do |source|
      filename = source.split('/').last.split('.')
      filename.pop
      target = "#{home}/.#{filename}"
      log shell "rm '#{target}'" if installed?(target.p)
      if "#{home}/.#{filename}.backup".p.exists?
        log shell "mv '#{home}/.#{filename}.backup' '#{home}/.#{filename}'"
      end
    end
  }
end
